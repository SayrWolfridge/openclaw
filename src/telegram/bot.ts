...
export function createTelegramBot(opts: TelegramBotOptions) {
  ...
  if (finalFetch) {
    ...
    type WrappedFetch = NonNullable<ApiClientOptions["fetch"]>;
    type WrappedFetchInput = Parameters<WrappedFetch>[0];
    type WrappedFetchInit = Parameters<WrappedFetch>[1];
    finalFetch = ((input: WrappedFetchInput, init?: WrappedFetchInit) => {
      return Promise.resolve(baseFetch(input, init)).catch((err: unknown) => {
        try {
          tagTelegramNetworkError(err, {
            method: extractTelegramApiMethod(input as RequestInfo | URL),
            url: readRequestUrl(input as RequestInfo | URL),
          });
        } catch {
